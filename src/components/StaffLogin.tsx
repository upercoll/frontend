...
export default function StaffLogin({ apiPath, tokenKey, dashboardPath, portalName, description }: StaffLoginProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await apiPost(apiPath, { email, password });
        ...
        <div className="flex-1 flex flex-col justify-center px-4 sm:px-12 py-10">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
            ...
            <div className="mb-6 text-center sm:text-left">
              ...
              <h2 className="text-2xl font-extrabold mb-1" style={{ color: "#F4F8FB" }}>{portalName}</h2>
              <p className="text-sm" style={{ color: "#637784" }}>Sign in to access your portal</p>
            </div>
            ...
            <p className="text-center text-xs mt-6" style={{ color: "#475569" }}>
              RBstars {portalName} — Authorized access only
            </p>
          </motion.div>
        </div>
      </div>
