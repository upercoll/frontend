  ...
  return (
    <div className="p-6 space-y-5 max-w-[1400px] mx-auto">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
        <div>
          <h2 className="text-xl font-bold" style={{ color: "#16204D" }}>Products</h2>
                  ...
                  <th className="px-4 py-3 w-10">
                    <button onClick={toggleAll}>
                      {selected.size === products.length && products.length > 0
                        ? <CheckSquare className="w-4 h-4" style={{ color: "#2B50F6" }} />
                  ...
                  return (
                    <tr key={p._id}
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <div className="flex items-center gap-1.5">
                              <p className="text-sm font-semibold" style={{ color: "#16204D" }}>{p.name}</p>
                      ...
                      <td className="px-4 py-3.5 hidden md:table-cell">
                        <span className="text-xs px-2 py-1 rounded-md font-medium" style={{ background: "#EEF2FF", color: "#2B50F6" }}>{p.game}</span>
                      </td>
                      <td className="px-4 py-3.5">
                        <div>
                          <p className="text-sm font-semibold" style={{ color: "#16204D" }}>${p.price.toFixed(2)}</p>
                      ...
                      <td className="px-4 py-3.5" onClick={e => e.stopPropagation()}>
                        {editingOnHand?.id === p._id ? (
                          <input
                            ...
                            style={{ border: "1px solid #2B50F6", color: "#16204D", background: "#fff" }}
                          />
                        ) : (
                          <button onClick={() => setEditingOnHand({ id: p._id, value: String(p.onHand ?? -1) })}
                            ...
                            <span className={`text-sm font-medium ${(p.onHand ?? -1) === 0 ? "text-red-500" : (p.onHand ?? -1) === -1 ? "text-slate-400" : ""}`}
                              style={(p.onHand ?? -1) > 0 ? { color: "#16204D" } : undefined}>
                        ...
                        {isEditingThisStock ? (
                          ...
                          <button onClick={() => setEditingStock({ id: p._id, value: String(p.stock) })}
                            ...
                            <span className={`text-sm font-medium ${p.stock === 0 ? "text-red-500" : p.stock === -1 ? "text-slate-400" : ""}`}
                              style={p.stock > 0 ? { color: "#16204D" } : undefined}>
                              {p.stock === -1 ? "∞" : p.stock}
                            </span>
                            <Edit2 className="w-3 h-3 text-slate-300 group-hover:text-indigo-400 transition-colors ml-0.5" />
                          </button>
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <motion.div initial={{ scale: 0.96, y: 16 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.96, y: 16 }}
              ...
              <div className="flex items-center justify-between px-6 py-4" style={{ borderBottom: "1px solid #F3F4F6" }}>
                <h3 className="font-bold text-base" style={{ color: "#16204D" }}>{modal === "create" ? "Add Product" : `Edit — ${editing?.name}`}</h3>
        ...
        {modal === "bulk" && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <motion.div initial={{ scale: 0.96, y: 16 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.96, y: 16 }}
              ...
              <div className="flex items-center justify-between px-6 py-4" style={{ borderBottom: "1px solid #F3F4F6" }}>
                <div>
                  <h3 className="font-bold text-base" style={{ color: "#16204D" }}>Quick Add Products</h3>
              ...
              <div className="p-6 space-y-5">
                <div className="rounded-xl p-4 space-y-3" style={{ background: "#F7F8FC", border: "1px solid #E9EBF5" }}>
                  ...
                  <button onClick={addToQueue}
                    ...
                    style={{ background: "#EEF2FF", color: "#2B50F6", border: "1px solid #c7d2fe" }}>
                        ...
                        return (
                          <div key={i} className="flex items-center gap-3 px-4 py-3 transition-colors hover:bg-slate-50"
                            ...
                            <div className="flex-1 min-w-0">
                              <p className="text-sm font-semibold truncate" style={{ color: "#16204D" }}>{item.name}</p>
