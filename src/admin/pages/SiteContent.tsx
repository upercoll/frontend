          ...
          <>
            {item.type === "richtext" ? (
              <textarea
                ...
                rows={4}
                className="w-full bg-[#101C48] border border-blue-500/30 text-white rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-blue-500/60 resize-none"
                autoFocus
              />
            ) : (
              <input
                value={String(edit.value || "")}
                onChange={(e) => setEditing((prev) => ({ ...prev, [item.key]: { ...prev[item.key], value: e.target.value } }))}
                className="w-full bg-[#101C48] border border-blue-500/30 text-white rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:border-blue-500/60"
                autoFocus
              />
            )}
            <div className="flex gap-2">
    ...
    <div className="p-6 space-y-4 max-w-[1000px] mx-auto">
      ...
      {isLoading ? (
        <div className="space-y-4">
          {Array.from({ length: 3 }).map((_, i) => <div key={i} className="h-40 bg-[#131F4D] rounded-xl border border-white/5 animate-pulse" />)}
        </div>
      ) : sections.length === 0 ? (
        ...
        <div className="space-y-3">
          {sections.map((section) => {
            ...
            return (
              <div key={section} className="bg-[#131F4D] border border-white/5 rounded-xl overflow-hidden">
