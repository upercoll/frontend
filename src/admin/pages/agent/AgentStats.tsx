    ...
    <div className="p-6 space-y-6 max-w-[1000px] mx-auto">
      ...
      {isLoading ? (
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
          {Array.from({ length: 6 }).map((_, i) => <div key={i} className="h-24 bg-[#131F4D] rounded-xl animate-pulse" />)}
        </div>
      ) : (
        <>
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
          ...
          {monthlyData.length > 0 && (
            <div className="bg-[#131F4D] border border-white/5 rounded-xl p-5">
              ...
              <ResponsiveContainer width="100%" height={200}>
                <BarChart data={monthlyData}>
                  ...
                  <Tooltip contentStyle={{ background: "#101C48", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 8, color: "#e2e8f0" }} />
                  <Bar dataKey="claims" name="Total" fill="#5A78FF" radius={[4, 4, 0, 0]} />
