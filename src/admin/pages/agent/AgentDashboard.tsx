...
const STATUS_COLORS: Record<string, { bg: string; text: string }> = {
  ...
  active:  { bg: "rgba(43,80,246,0.12)",  text: "#B9C6FF" },
...
export default function AgentDashboard() {
  ...
  const AGENT_STAT_CONFIGS = [
    { label: "Claims Done", value: agentStats?.completedClaims || 0, color: "#22C55E", format: String },
    { label: "Completion Rate", value: `${completionRate}%`, color: "#2B50F6" },
    { label: "Avg Response", value: msToTime(agentStats?.avgResponseTimeMs || 0), color: "#f59e0b" },
    { label: "Rating", value: agentStats?.rating?.count ? `${agentStats.rating.average.toFixed(1)}★` : "—", color: "#B9A6FF" },
  ];

  const ADMIN_STAT_CONFIGS = [
    { key: "totalRevenue",   label: "Total Revenue",  color: "#22C55E", format: (v: number) => `$${v.toFixed(2)}`, icon: DollarSign },
    { key: "ordersToday",    label: "Orders Today",   color: "#2B50F6", format: String,                             icon: ShoppingBag },
    { key: "totalOrders",    label: "Total Orders",   color: "#7C5CFF", format: String,                             icon: Zap },
  ...
  return (
    <div className="p-4 sm:p-6 space-y-6 max-w-[1200px] mx-auto">
      ...
      <div className="flex items-end justify-between">
        <div>
          <p className="text-xs font-bold uppercase tracking-widest mb-1" style={{ color: "rgba(43,80,246,0.7)" }}>
            Team Panel
          </p>
        ...
        <div>
          <SectionHeading icon={BarChart3} title="Overview" action={
            <Link href="/admin/analytics">
              <span className="flex items-center gap-1 text-xs font-semibold cursor-pointer hover:opacity-80" style={{ color: "#B9C6FF" }}>
          ...
          {canViewClaims && (
            <Link href="/panel/queue">
              <motion.div whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}
                ...
                style={{ background: "linear-gradient(135deg,rgba(43,80,246,0.25) 0%,rgba(124,92,255,0.15) 100%)", border: "1px solid rgba(43,80,246,0.3)" }}>
                <MessageSquare className="w-8 h-8 mb-3" style={{ color: "#B9C6FF" }} />
                ...
                <tbody>
                  {recentOrders.map((order: any, i: number) => {
                    const statusColors: Record<string, { bg: string; text: string }> = {
                      pending: { bg: "rgba(245,158,11,0.12)", text: "#fbbf24" },
                      paid: { bg: "rgba(43,80,246,0.12)", text: "#B9C6FF" },
                      delivering: { bg: "rgba(124,92,255,0.12)", text: "#C7D4FF" },
                      ...
                      <tr key={order._id} className="transition-colors" style={{ borderBottom: "1px solid rgba(255,255,255,0.04)" }}>
                        <td className="px-4 py-3.5">
                          <Link href={`/admin/orders/${order._id}`}>
                            <span className="text-sm font-mono font-semibold cursor-pointer hover:opacity-80" style={{ color: "#B9C6FF" }}>
                              {order.orderNumber}
                            </span>
                          </Link>
                        </td>
                        <td className="px-4 py-3.5">
                          <div className="flex items-center gap-2">
                            <div className="w-7 h-7 rounded-lg flex-shrink-0 overflow-hidden"
                              style={{ background: order.items?.[0]?.productSnapshot?.gradient ? `linear-gradient(135deg,${order.items[0].productSnapshot.gradient.from},${order.items[0].productSnapshot.gradient.to})` : "rgba(43,80,246,0.15)" }}>
              ...
              return (
                <motion.div key={session._id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: i * 0.04 }}
                  ...
                  <div className="w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0"
                    style={{ background: "rgba(43,80,246,0.1)" }}>
                    <MessageSquare className="w-4 h-4" style={{ color: "#B9C6FF" }} />
                  </div>
