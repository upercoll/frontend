  ...
  return (
    <div className="flex items-center gap-3 mb-5">
      <div className="w-7 h-7 rounded-lg flex items-center justify-center flex-shrink-0"
        style={{ background: "rgba(43,80,246,0.15)", border: "1px solid rgba(43,80,246,0.2)" }}>
        <Icon className="w-3.5 h-3.5" style={{ color: "#B9C6FF" }} />
      </div>
      <h2 className="font-bold text-sm uppercase tracking-widest" style={{ color: "rgba(255,255,255,0.5)" }}>{title}</h2>
      <div className="flex-1 h-px" style={{ background: "linear-gradient(90deg, rgba(43,80,246,0.2), transparent)" }} />
      {action}
    </div>
  );
}
...
const STATUS_COLORS: Record<string, { bg: string; text: string; dot: string }> = {
  ...
  paid:               { bg: "rgba(43,80,246,0.12)", text: "#B9C6FF", dot: "#2B50F6" },
  delivering:         { bg: "rgba(124,92,255,0.12)", text: "#C7D4FF", dot: "#7C5CFF" },
  completed:          { bg: "rgba(16,185,129,0.12)", text: "#6ee7b7", dot: "#22C55E" },
...
const STAT_CONFIGS = [
  { key: "totalRevenue",    label: "Total Revenue",   icon: DollarSign,    color: "#22C55E", format: (v: number) => `$${v.toFixed(2)}` },
  { key: "revenueThisMonth", label: "This Month",     icon: TrendingUp,    color: "#2B50F6", format: (v: number) => `$${v.toFixed(2)}` },
  { key: "ordersToday",     label: "Orders Today",    icon: ShoppingBag,   color: "#7C5CFF", format: (v: number) => String(v) },
  ...
  { key: "totalCustomers",  label: "Customers",       icon: Users,         color: "#B9A6FF", format: (v: number) => String(v) },
];
  ...
  return (
    <div className="p-6 space-y-8 max-w-[1400px] mx-auto">
      ...
      <div className="flex items-end justify-between">
        <div>
          <p className="text-xs font-bold uppercase tracking-widest mb-1" style={{ color: "rgba(124,92,255,0.7)" }}>
            Admin Panel
          </p>
        ...
        <div className="hidden sm:flex items-center gap-2 px-3 py-2 rounded-xl"
          style={{ background: "rgba(43,80,246,0.1)", border: "1px solid rgba(43,80,246,0.2)" }}>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
        ...
        >
          ...
          <div className="space-y-3">
            {[
              { label: "Completion Rate", value: stats?.totalOrders ? `${Math.round(((stats.totalOrders - (stats.pendingClaims || 0)) / stats.totalOrders) * 100)}%` : "—", color: "#22C55E" },
              { label: "Avg Order Value", value: stats?.totalOrders ? `$${((stats.totalRevenue || 0) / stats.totalOrders).toFixed(2)}` : "—", color: "#2B50F6" },
              { label: "Monthly Orders", value: String(stats?.ordersThisMonth || 0), color: "#7C5CFF" },
              { label: "Revenue Growth", value: `${stats?.revenueGrowth?.toFixed(1) || 0}%`, color: (stats?.revenueGrowth || 0) >= 0 ? "#22C55E" : "#ef4444" },
            ].map((item, i) => (
          ...
          <SectionHeading
            ...
            action={
              <Link href="/admin/orders">
                <span className="flex items-center gap-1 text-xs font-semibold cursor-pointer hover:opacity-80 transition-opacity" style={{ color: "#B9C6FF" }}>
                  ...
                  >
                    <td className="px-4 py-3.5">
                      <Link href={`/admin/orders/${order._id}`}>
                        <span className="text-sm font-mono font-semibold cursor-pointer hover:opacity-80 transition-opacity" style={{ color: "#B9C6FF" }}>
                          {order.orderNumber}
                        </span>
                      </Link>
                    </td>
