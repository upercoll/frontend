...
function StatCard({ icon: Icon, label, value, growth, prefix = "", color = "#2B50F6" }: {
  ...
  return (
    <div className="bg-white rounded-xl p-5 transition-shadow hover:shadow-md" style={{ border: "1px solid #E9EBF5" }}>
      ...
      <p className="text-2xl font-bold" style={{ color: "#16204D" }}>{prefix}{typeof value === "number" ? value.toLocaleString("en-US", { maximumFractionDigits: 2 }) : value}</p>
  ...
  return (
    <div className="p-6 space-y-6 max-w-[1400px] mx-auto">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
        <div>
          <h2 className="text-xl font-bold" style={{ color: "#16204D" }}>Analytics</h2>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard
          ...
          growth={summary?.revenueGrowth} prefix="$" color="#2B50F6"
        />
        <StatCard
      ...
      {conversion && (
        <div className="grid grid-cols-3 gap-4">
          <div className="bg-white rounded-xl p-5" style={{ border: "1px solid #E9EBF5" }}>
            ...
            <p className="text-2xl font-bold" style={{ color: "#16204D" }}>{conversion.totalOrders.toLocaleString()}</p>
          </div>
          <div className="bg-white rounded-xl p-5" style={{ border: "1px solid #E9EBF5" }}>
      ...
      {summary?.statusBreakdown && (
        <div className="bg-white rounded-xl p-5" style={{ border: "1px solid #E9EBF5" }}>
          <h3 className="text-sm font-bold mb-4" style={{ color: "#16204D" }}>Order Status Breakdown</h3>
          <div className="flex flex-wrap gap-3">
            {Object.entries(summary.statusBreakdown).map(([status, count]) => (
              <div key={status} className="flex items-center gap-2 px-3 py-2 rounded-lg" style={{ background: "#F7F8FC", border: "1px solid #E9EBF5" }}>
                <span className="text-sm font-semibold capitalize" style={{ color: "#16204D" }}>{status === "pending" ? "Unpaid" : status}</span>
      ...
      <div className="bg-white rounded-xl p-5" style={{ border: "1px solid #E9EBF5" }}>
        <div className="flex items-center justify-between mb-5">
          <h3 className="text-sm font-bold" style={{ color: "#16204D" }}>Revenue Chart</h3>
          <div className="flex gap-1 p-1 rounded-lg" style={{ background: "#F7F8FC", border: "1px solid #E9EBF5" }}>
            {(["monthly", "daily"] as const).map((p) => (
              <button
                ...
                style={chartPeriod === p ? { background: "#16204D", color: "#fff" } : { color: "#6b7280" }}
              >
            ...
            {chart.map((point: any, i: number) => {
              ...
              return (
                <motion.div
                  ...
                  style={{ background: "linear-gradient(to top, #2B50F6, #93A8FF)", minHeight: point.revenue > 0 ? 2 : 0 }}
                >
                  {point.revenue > 0 && (
                    <div className="absolute -top-8 left-1/2 -translate-x-1/2 hidden group-hover:block bg-[#16204D] text-white text-[10px] rounded px-2 py-1 whitespace-nowrap z-10 shadow-lg">
        ...
        <div className="bg-white rounded-xl p-5" style={{ border: "1px solid #E9EBF5" }}>
          <div className="flex items-center justify-between mb-5">
            <div className="flex items-center gap-2">
              ...
              <div>
                <h3 className="text-sm font-bold" style={{ color: "#16204D" }}>Site Traffic (Last 30 Days)</h3>
            ...
            <div className="flex gap-4 text-right">
              ...
              <div>
                <p className="text-lg font-bold" style={{ color: "#2B50F6" }}>{trafficSummary.orderAttempts30d.toLocaleString()}</p>
                <p className="text-[10px] text-slate-400">Order Attempts</p>
              </div>
              <div>
                <p className="text-lg font-bold" style={{ color: "#16204D" }}>{trafficSummary.totalCustomers.toLocaleString()}</p>
                  ...
                  return (
                    <div key={i} className="flex-1 flex items-end gap-px relative group">
                      ...
                      {(point.newCustomers > 0 || point.orderAttempts > 0) && (
                        <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-1 hidden group-hover:block bg-[#16204D] text-white text-[10px] rounded px-2 py-1 whitespace-nowrap z-10 shadow-lg pointer-events-none">
                          <div>{point.label}</div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        <div className="bg-white rounded-xl p-5" style={{ border: "1px solid #E9EBF5" }}>
          <h3 className="text-sm font-bold mb-4" style={{ color: "#16204D" }}>Revenue by Game</h3>
          {byGame.length === 0 ? (
            ...
            <div className="space-y-3">
              {byGame.map((g: any, i: number) => {
                ...
                const colors = ["#2B50F6", "#0ea5e9", "#7C5CFF", "#22C55E", "#f59e0b", "#ef4444"];
                const col = colors[i % colors.length];
                return (
                  <div key={g._id || i}>
                    <div className="flex items-center justify-between mb-1">
                      <span className="text-sm font-medium" style={{ color: "#374151" }}>{g._id || "Unknown"}</span>
                      <span className="text-sm font-semibold" style={{ color: "#16204D" }}>${g.revenue.toFixed(2)}</span>
                    </div>
        ...
        <div className="bg-white rounded-xl p-5" style={{ border: "1px solid #E9EBF5" }}>
          <h3 className="text-sm font-bold mb-4" style={{ color: "#16204D" }}>Top Products</h3>
              ...
              {topProducts.slice(0, 8).map((p: any, i: number) => (
                <div key={p._id || i} className="flex items-center gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium truncate" style={{ color: "#16204D" }}>{p.name}</p>
