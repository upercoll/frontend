...
export default function Games() {
  ...
  const openCreate = () => {
    setGameName(""); setGameDesc(""); setGameGradFrom("#1e3a5f"); setGameGradTo("#0f172a"); setGameImage(null); setGameBgImageUrl(""); setGameFeatured(false); setGameClaimTime(0); setGameClaimSchedule([]); setAutoEnabled(false); setAutoPrivateServerUrl(""); setAutoInstructions(""); setError("");
    setEditingGame(null);
    ...
    <div className="p-4 md:p-6 space-y-5 max-w-[1200px] mx-auto">
      ...
      {isLoading ? (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {Array.from({ length: 4 }).map((_, i) => <div key={i} className="h-32 bg-[#131F4D] rounded-xl border border-white/5 animate-pulse" />)}
        </div>
      ) : games.length === 0 ? (
          ...
          {games.map((game: Game, i: number) => {
            ...
            return (
              <motion.div
                ...
                className="bg-[#131F4D] border border-white/5 rounded-xl overflow-hidden"
              >
                                  ...
                                  {addingSubcatFor === cat._id && (
                                    <div className="flex gap-2 mt-2">
                                      <input
                                        ...
                                        className="flex-1 bg-[#101C48] border border-blue-500/40 text-white placeholder-slate-600 rounded-lg px-3 py-1.5 text-xs focus:outline-none"
                                      />
                                      <button
        ...
        {deleteConfirm && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <motion.div
              ...
              className="bg-[#131F4D] border border-white/10 rounded-t-2xl sm:rounded-2xl w-full sm:max-w-sm overflow-hidden"
              ...
              <form onSubmit={handleGameSubmit} className="p-6 space-y-4">
                <div>
                  ...
                  <input value={gameName} onChange={(e) => setGameName(e.target.value)} required placeholder="e.g. Murder Mystery 2"
                    className="w-full bg-[#101C48] border border-white/10 text-white placeholder-slate-600 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:border-blue-500/50" />
                </div>
                <div>
                  ...
                  <div>
                    ...
                    <div className="flex gap-2">
                      ...
                      <input value={gameGradFrom} onChange={(e) => setGameGradFrom(e.target.value)}
                        className="flex-1 bg-[#101C48] border border-white/10 text-white rounded-xl px-3 py-2 text-sm focus:outline-none focus:border-blue-500/50 min-w-0" />
                    </div>
                  </div>
                  <div>
                ...
                <div>
                  ...
                  <input type="file" accept="image/*" onChange={(e) => setGameImage(e.target.files?.[0] || null)}
                    className="w-full bg-[#101C48] border border-white/10 text-slate-400 rounded-xl px-4 py-2 text-sm focus:outline-none" />
                </div>
                <div>
                  <ImageUpload
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full bg-[#101C48] border border-white/10 text-white rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:border-blue-500/50"
                  />
                  ...
                  {gameClaimSchedule.length > 0 && (
                    <div className="space-y-2">
                      {gameClaimSchedule.map((slot, idx) => (
                        <div key={idx} className="bg-[#101C48] border border-white/8 rounded-xl p-3 space-y-2">
