...
const ACTION_STYLE: Record<string, { label: string; color: string; bg: string }> = {
  ...
  info: { label: "INFO", color: "#B9A6FF", bg: "rgba(167,139,250,0.12)" },
};
    ...
    <div className="p-6 space-y-5 max-w-[1300px] mx-auto">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          <div className="flex items-center gap-2 bg-[#131F4D] border border-white/10 rounded-xl px-3 py-2">
          ...
          <button
            ...
            className={cn("px-3 py-2 rounded-xl text-xs font-bold border transition-colors",
              autoRefresh
                ...
                : "bg-[#131F4D] border-white/10 text-slate-400")}
          >
            AUTO REFRESH {autoRefresh ? "ON" : "OFF"}
          </button>
          <button
            onClick={() => refetch()}
            className="px-3 py-2 rounded-xl text-xs font-bold bg-[#131F4D] border border-white/10 text-slate-300 hover:border-white/20 transition-colors flex items-center gap-1.5"
          >
      ...
      {isLoading ? (
        <div className="space-y-2">
          {Array.from({ length: 8 }).map((_, i) => (
            <div key={i} className="h-14 bg-[#131F4D] rounded-xl border border-white/5 animate-pulse" />
          ))}
        </div>
      ) : logs.length === 0 ? (
            ...
            return (
              <motion.div
                ...
                className={cn("rounded-xl border p-3.5 flex flex-col md:flex-row md:items-center gap-3",
                  isError ? "bg-red-500/5 border-red-500/20" : "bg-[#131F4D] border-white/5")}
              >
