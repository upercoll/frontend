  ...
  return (
    ...
    >
      <div className="relative flex-shrink-0 mt-0.5">
        ...
        <span className={cn("absolute -bottom-0.5 -right-0.5 w-2.5 h-2.5 rounded-full border-[2px] border-[#101C48]", cfg.dot, cfg.pulse ? "animate-pulse" : "")} />
      </div>
...
function ProfilePanel({ session, isMyActive, onClose, onDeliver, onEnd, onCloseChat }: {
  ...
  return (
    <div className="flex flex-col h-full bg-[#101C48] border-l border-white/5 overflow-y-auto">
      {/* Header */}
        ...
        <div className="px-4 py-4 border-b border-white/5 flex-shrink-0">
          ...
          <div className="grid grid-cols-2 gap-2">
            {orderItems.map((item, i) => (
              <div key={i} className="bg-[#131F4D] border border-white/5 rounded-xl overflow-hidden flex flex-col">
                <div
                  className="relative w-full h-16 overflow-hidden flex-shrink-0"
                  style={{ background: item.gradient ? `linear-gradient(135deg, ${item.gradient.from} 0%, ${item.gradient.to} 100%)` : "linear-gradient(135deg, #16204D 0%, #312e81 100%)" }}
                >
...
export default function DelivererQueue() {
  ...
  return (
    <div className="flex h-[calc(100vh-64px-48px)] overflow-hidden rounded-2xl" style={{ background: "rgba(6,9,28,0.8)", border: "1px solid rgba(255,255,255,0.06)" }}>
      ...
      {/* ══════ LEFT: Inbox ══════ */}
      <div className={cn("flex flex-col flex-shrink-0 border-r border-white/5 bg-[#101C48] w-full md:w-72", mobilePanel !== 0 ? "hidden md:flex" : "flex")}>
      ...
      <div className={cn("flex flex-col flex-1 min-w-0 overflow-hidden", mobilePanel === 1 ? "flex" : "hidden md:flex")}>
        {!selectedSession ? (
          ...
          <>
            {/* Chat header */}
            <div className="flex items-center gap-2 px-3 py-2.5 border-b border-white/5 flex-shrink-0 bg-[#101C48]">
            ...
            {/* Mobile action bar */}
            {selectedSession.status === "pending" && (
              <div className="sm:hidden px-3 py-2 border-b border-white/5 bg-[#101C48] flex-shrink-0">
                <button onClick={() => claimSession(selectedSession.roomId)} disabled={!!claiming}
            ...
            {isMyActive && (
              <div className="sm:hidden px-3 py-2 border-b border-white/5 bg-[#101C48] flex-shrink-0 flex gap-2">
            ...
            {(selectedSession.status === "claimed" || selectedSession.status === "ended") && (
              <div className="px-4 py-3 border-t border-white/5 flex-shrink-0">
                <div className="flex items-center gap-2 px-3 py-2 rounded-xl" style={{
                  ...
                  <CheckCircle className="w-4 h-4 flex-shrink-0" style={{ color: selectedSession.status === "claimed" ? "#4ade80" : "#B9A6FF" }} />
                  <p className="text-xs font-medium" style={{ color: selectedSession.status === "claimed" ? "#4ade80" : "#B9A6FF" }}>
      ...
      {mobilePanel === 2 && (
        <button onClick={() => setMobilePanel(1)} className="md:hidden fixed top-[72px] left-3 z-30 w-8 h-8 bg-[#101C48] border border-white/10 rounded-lg flex items-center justify-center text-slate-400">
          <ArrowLeft className="w-4 h-4" />
        </button>
      )}
        ...
        {podMode && selectedSession && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <motion.div initial={{ scale: 0.95, y: 20 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.95, y: 20 }}
              className="bg-[#131F4D] border border-white/10 rounded-t-2xl sm:rounded-2xl w-full sm:max-w-md overflow-hidden">
              ...
              <div className="p-6 space-y-4">
                ...
                <div>
                  ...
                  <textarea value={podNotes} onChange={e => setPodNotes(e.target.value)} placeholder="Optional delivery notes…" rows={2}
                    className="w-full bg-[#101C48] border border-white/10 text-white placeholder-slate-600 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:border-sky-500/50 resize-none" />
                </div>
              </div>
        ...
        {closingSession && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <motion.div initial={{ scale: 0.95, y: 12 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.95, y: 12 }}
              ...
              className="bg-[#131F4D] border border-white/10 rounded-2xl w-full max-w-sm overflow-hidden">
