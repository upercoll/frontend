...
function GameClaimCard({ game, idx }: { game: Game; idx: number }) {
  ...
  return (
    <motion.div
      ...
      className="bg-[#131F4D] border border-white/5 rounded-xl p-4 space-y-4"
    >
      <div className="flex items-center gap-3">
        {game.imageUrl ? (
          ...
          <div
            ...
            style={{ background: `linear-gradient(135deg, ${game.gradient?.from ?? "#2B50F6"}, ${game.gradient?.to ?? "#7C5CFF"})` }}
          >
      ...
      <div className="space-y-2">
        {slots.map((slot, i) => (
          <div key={i} className="flex items-center gap-2 bg-[#101C48] border border-white/8 rounded-xl px-3 py-2.5">
  ...
  return (
    <div className="p-4 md:p-6 max-w-2xl mx-auto space-y-4">
      ...
      <div
        ...
        style={{ background: "rgba(43,80,246,0.08)", border: "1px solid rgba(43,80,246,0.15)", color: "#B9C6FF" }}
      >
      ...
      {isLoading ? (
        <div className="space-y-3">
          {Array.from({ length: 3 }).map((_, i) => (
            <div key={i} className="h-28 bg-[#131F4D] rounded-xl border border-white/5 animate-pulse" />
          ))}
        </div>
