...
export default function Customers() {
  ...
  return (
    <div className="p-6 space-y-5 max-w-[1400px] mx-auto">
      <div>
        <h2 className="text-xl font-bold" style={{ color: "#16204D" }}>Customers</h2>
      ...
      {stats && (
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          {[
            { icon: Users, label: "Total Customers", value: stats.total, color: "#2B50F6" },
            { icon: Users, label: "Active Customers", value: stats.active, color: "#22C55E" },
            { icon: TrendingUp, label: "New This Month", value: stats.newThisMonth, color: "#f59e0b" },
            { icon: DollarSign, label: "Top Spender", value: stats.topSpenders?.[0] ? `$${stats.topSpenders[0].total.toFixed(2)}` : "-", color: "#7C5CFF" },
            ...
            <div key={i} className="bg-white rounded-xl p-4" style={{ border: "1px solid #E9EBF5" }}>
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-xl font-bold" style={{ color: "#16204D" }}>{typeof value === "number" ? value.toLocaleString() : value}</p>
      ...
      {stats?.topSpenders && stats.topSpenders.length > 0 && (
        <div className="bg-white rounded-xl p-5" style={{ border: "1px solid #E9EBF5" }}>
          <h3 className="text-sm font-bold mb-4" style={{ color: "#16204D" }}>Top Spenders</h3>
            ...
            {stats.topSpenders.map((s: any, i: number) => (
              <div key={i} className="flex items-center gap-3">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium" style={{ color: "#16204D" }}>{s.robloxUsername || s._id}</p>
                ...
                {customers.map((customer: any) => (
                  ...
                  >
                    <td className="px-5 py-3.5">
                      <div className="flex items-center gap-3">
                        <div className="w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0" style={{ background: "linear-gradient(135deg,#2B50F6,#2B50F6)" }}>
                          <span className="text-white text-xs font-bold">{customer.displayName[0]?.toUpperCase()}</span>
                        </div>
                        <div>
                          <p className="text-sm font-medium" style={{ color: "#16204D" }}>{customer.displayName}</p>
                    ...
                    <td className="px-5 py-3.5">
                      <div className="flex items-center gap-1">
                        ...
                        <span className="text-sm font-medium" style={{ color: "#16204D" }}>{customer.orderCount || 0}</span>
                      </div>
                    </td>
                    <td className="px-5 py-3.5">
                    ...
                    <td className="px-5 py-3.5">
                      <button
                        ...
                        style={{ background: customer.active !== false ? "#FEF2F2" : "#ECFDF5", color: customer.active !== false ? "#EF4444" : "#22C55E" }}
