  ...
  return (
    <div className="p-6 space-y-5 max-w-[1200px] mx-auto">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h2 className="text-xl font-bold" style={{ color: "#16204D" }}>{collab.name}</h2>
      ...
      <div className="grid grid-cols-3 gap-4">
        <div className="bg-white rounded-xl p-5" style={{ border: "1px solid #E9EBF5" }}>
          ...
          <p className="text-2xl font-bold mt-1" style={{ color: "#16204D" }}>{products.length}</p>
        </div>
        <div className="bg-white rounded-xl p-5" style={{ border: "1px solid #E9EBF5" }}>
        ...
        <Link href={`/admin/collaboration/payouts/${id}`}>
          <div className="bg-white rounded-xl p-5 cursor-pointer transition-colors hover:bg-slate-50" style={{ border: "1px solid #E9EBF5" }}>
            ...
            <p className="text-sm font-semibold mt-1" style={{ color: "#16204D" }}>
      ...
      <div className="bg-white rounded-xl overflow-hidden" style={{ border: "1px solid #E9EBF5", boxShadow: "0 1px 3px rgba(0,0,0,0.04)" }}>
        <div className="flex items-center justify-between px-5 py-4" style={{ borderBottom: "1px solid #F3F4F6" }}>
          <div>
            <h3 className="font-bold text-sm" style={{ color: "#16204D" }}>Connected Products ({products.length})</h3>
          </div>
                  ...
                  <td className="px-5 py-3.5">
                    <div className="flex items-center gap-3">
                      <div className="w-9 h-9 rounded-lg flex-shrink-0 overflow-hidden"
                        style={{ background: cp.product?.gradient ? `linear-gradient(135deg, ${cp.product.gradient.from}, ${cp.product.gradient.to})` : "#2B50F6" }}>
                        {cp.product?.imageUrl
                      ...
                      <div>
                        <p className="text-sm font-semibold" style={{ color: "#16204D" }}>{cp.productName || cp.product?.name}</p>
                        <p className="text-xs text-slate-400">{cp.product?.game || ""}</p>
                      </div>
                    </div>
                  </td>
                  <td className="px-5 py-3.5">
                    <span className="text-sm font-semibold" style={{ color: "#16204D" }}>{cp.cut}%</span>
                  </td>
                  <td className="px-5 py-3.5">
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <motion.div initial={{ scale: 0.96, y: 16 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.96, y: 16 }}
              ...
              <div className="flex items-center justify-between px-6 py-4 flex-shrink-0" style={{ borderBottom: "1px solid #F3F4F6" }}>
                <div>
                  <h3 className="font-bold text-base" style={{ color: "#16204D" }}>Bulk Add Products</h3>
                      ...
                      {availableProducts.map((p: any) => {
                        ...
                        return (
                          <div
                            ...
                            style={{
                              border: isSelected ? "1px solid #2B50F6" : "1px solid #E9EBF5",
                          ...
                          >
                            ...
                            <div className="w-7 h-7 rounded overflow-hidden flex-shrink-0"
                              style={{ background: p.gradient ? `linear-gradient(135deg, ${p.gradient.from}, ${p.gradient.to})` : "#2B50F6" }}>
                              {p.imageUrl
                                ? <img src={p.imageUrl} className="w-full h-full object-cover" alt="" />
                                : <div className="w-full h-full flex items-center justify-center"><Package className="w-3 h-3 text-white/60" /></div>}
                            </div>
                            <div className="flex-1 min-w-0">
                              <p className="text-sm font-semibold truncate" style={{ color: "#16204D" }}>{p.name}</p>
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <motion.div initial={{ scale: 0.96, y: 16 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.96, y: 16 }}
              ...
              <div className="flex items-center justify-between px-6 py-4" style={{ borderBottom: "1px solid #F3F4F6" }}>
                <h3 className="font-bold text-base" style={{ color: "#16204D" }}>Connect Product</h3>
        ...
        {editModal && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <motion.div initial={{ scale: 0.96, y: 16 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.96, y: 16 }}
              ...
              onClick={e => e.stopPropagation()}>
              <div className="flex items-center justify-between px-6 py-4" style={{ borderBottom: "1px solid #F3F4F6" }}>
                <h3 className="font-bold text-base" style={{ color: "#16204D" }}>Edit Cut — {editModal.productName}</h3>
