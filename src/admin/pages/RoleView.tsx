    ...
    <div className="p-6 space-y-6 max-w-[1200px] mx-auto">
      ...
      <div className="relative w-72">
        <button
          ...
          className="w-full flex items-center justify-between gap-3 px-4 py-3 bg-[#131F4D] border border-white/10 rounded-xl text-sm text-white hover:border-blue-500/40 transition-colors"
        >
          {selectedRole ? (
            <div className="flex items-center gap-2">
              <span
                className="w-2.5 h-2.5 rounded-full flex-shrink-0"
                style={{ background: selectedRole.color || "#5A78FF" }}
              />
        ...
        <AnimatePresence>
          {dropdownOpen && (
            <motion.div
              ...
              exit={{ opacity: 0, y: -4 }}
              className="absolute top-full mt-1 w-full bg-[#131F4D] border border-white/10 rounded-xl overflow-hidden z-20 shadow-xl"
            >
              {roles.map((role) => (
                <button
                  key={role._id}
                ...
                >
                  <span className="w-2.5 h-2.5 rounded-full flex-shrink-0" style={{ background: role.color || "#5A78FF" }} />
