    ...
    <div className="p-6 space-y-5 max-w-[1200px] mx-auto">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
        ...
        <select value={viewedFilter} onChange={(e) => { setViewedFilter(e.target.value); setPage(1); }}
          className="bg-[#131F4D] border border-white/10 text-slate-300 rounded-xl px-3 py-2 text-sm focus:outline-none">
      ...
      {isLoading ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {Array.from({ length: 6 }).map((_, i) => <div key={i} className="h-48 bg-[#131F4D] rounded-xl border border-white/5 animate-pulse" />)}
        </div>
      ) : proofs.length === 0 ? (
          ...
          {proofs.map((proof: ProofOfDelivery, i: number) => {
            ...
            return (
              <motion.div key={proof._id} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.04 }}
                className={cn("bg-[#131F4D] border rounded-xl overflow-hidden cursor-pointer hover:border-white/15 transition-colors group",
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex gap-2">
            <button onClick={() => setPage((p) => Math.max(1, p - 1))} disabled={page === 1}
              className="w-8 h-8 rounded-lg bg-[#131F4D] border border-white/5 disabled:opacity-30 flex items-center justify-center text-slate-400 hover:text-white">
          ...
          return (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
              ...
              <motion.div initial={{ scale: 0.95, y: 20 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.95, y: 20 }}
                className="bg-[#131F4D] border border-white/10 rounded-2xl w-full max-w-2xl overflow-hidden max-h-[90vh] flex flex-col"
