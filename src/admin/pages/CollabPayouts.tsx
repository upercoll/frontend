  ...
  return (
    <div className="p-6 space-y-5 max-w-[1000px] mx-auto">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h2 className="text-xl font-bold" style={{ color: "#16204D" }}>Payouts — {collab?.name}</h2>
            ...
            <tbody>
              {payouts.map((p: any, idx: number) => (
                <tr key={p._id} style={{ borderBottom: "1px solid #F3F4F6" }}
                  ...
                  <td className="px-5 py-3.5 text-sm font-medium" style={{ color: "#16204D" }}>Payment to {collab?.name}</td>
                  ...
                  <td className="px-5 py-3.5">
                    <Link href={`/admin/collaboration/payouts/${id}/detail/${p._id}`}>
                      <button className="w-7 h-7 rounded-lg flex items-center justify-center" style={{ background: "#EEF2FF", color: "#2B50F6" }}>
