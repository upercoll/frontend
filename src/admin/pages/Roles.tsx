...
const COLORS = ["#2B50F6", "#5A78FF", "#7C5CFF", "#ec4899", "#22C55E", "#f59e0b", "#ef4444", "#06b6d4"];
      ...
      {isLoading ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {Array.from({ length: 4 }).map((_, i) => (
            <div key={i} className="h-40 bg-[#131F4D] rounded-xl border border-white/5 animate-pulse" />
          ))}
        </div>
      ) : roles.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {roles.map((role: AdminRole, i: number) => (
            <motion.div
              ...
              className="bg-[#131F4D] border border-white/5 rounded-xl p-5 hover:border-white/10 transition-colors group"
            >
        ...
        {modal.mode && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <motion.div
              ...
              className="bg-[#131F4D] border border-white/10 rounded-2xl w-full max-w-2xl my-8 overflow-hidden"
                  ...
                  <div>
                    ...
                    <div className="flex gap-2 mb-2">
                      <input
                        ...
                        className="flex-1 bg-[#101C48] border border-white/10 text-white placeholder-slate-600 rounded-xl px-4 py-2 text-sm focus:outline-none focus:border-blue-500/50"
                      />
        ...
        {deleteConfirm && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <motion.div initial={{ scale: 0.9 }} animate={{ scale: 1 }} exit={{ scale: 0.9 }}
              className="bg-[#131F4D] border border-white/10 rounded-2xl p-6 w-full max-w-sm"
              onClick={(e) => e.stopPropagation()}>
