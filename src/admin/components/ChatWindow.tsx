...
export default function ChatWindow({ session, onUpdate, onSessionClaimed }: ChatWindowProps) {
  ...
  return (
    <div className="flex flex-col h-full bg-[#101C48] rounded-xl border border-white/5 overflow-hidden">
      {/* Header */}
      <div className="px-4 py-3 border-b border-white/5 bg-[#131F4D] flex items-center gap-3 flex-shrink-0">
      ...
      <div className="flex-1 overflow-y-auto p-3 md:p-4 space-y-3">
                ...
                <div className={cn(
                  ...
                  <div className={cn(
                    ...
                    msg.sender === "agent"
                      ...
                      : "bg-[#131F4D] text-slate-200 border border-white/5 rounded-bl-sm"
                  )}>
                    {msg.text}
                  </div>
        ...
        {isTyping && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex gap-1 items-center">
            <div className="flex gap-1 bg-[#131F4D] border border-white/5 px-3 py-2 rounded-2xl rounded-bl-sm">
              {[0, 1, 2].map(i => (
                <motion.div
                  key={i}
                  animate={{ y: [0, -4, 0] }}
